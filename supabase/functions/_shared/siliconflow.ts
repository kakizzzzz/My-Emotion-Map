...
const readProviderConfig = (task: 'photo' | 'chat') => {
  ...
  const configuredModel = Deno.env.get(
    task === 'photo' ? 'SILICONFLOW_PHOTO_MODEL' : 'SILICONFLOW_CHAT_MODEL',
  )?.trim() ?? '';
  const requiredModel = task === 'photo' ? PHOTO_MODEL : CHAT_MODEL;
