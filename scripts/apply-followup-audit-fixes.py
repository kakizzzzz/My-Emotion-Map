...
def replace_regex(path: str, pattern: str, replacement: str, label: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f'{path} [{label}]: expected one regex replacement, found {count}')
    file_path.write_text(next_source, encoding='utf-8')


Path('src/app/useFollowUpScheduler.ts').write_text("""import {
  useCallback,
  useEffect,
  useReducer,
  type Dispatch,
...
Path('src/app/noteEditorHandlers.ts').write_text("""import type { Dispatch, SetStateAction } from 'react';
...
replace_once(
    ...
    'revisit target state',
)
replace_once(
    app_path,
    "    starSavedMessage: copy.feedback.starSaved,\n    followUps,",
    "    starSavedMessage: copy.feedback.starSaved,\n    notes,\n    followUps,",
    'pass notes to editor handler',
)
replace_regex(
    app_path,
    r"  const updateRevisitedEmotion = \(noteId: string, emotion: EmotionKey\) => \{.*?\n  \};\n\n  if \(!cloudSession\.session",
  ...
  if (!cloudSession.session""",
    'target exact answered follow-up',
)
replace_once(
    app_path,
    ...
    'chat revisit callback',
)
replace_regex(
    app_path,
    r"                    if \(kind !== 'skip' && followUp\) \{\n\s*window\.setTimeout\(\(\) => setRevisitNoteId\(followUp\.noteId\), 220\);\n\s*\}",
...
replace_once(
    ...
    """              onClose={() => {
              ...
              onConfirm={updateRevisitedEmotion}""",
    'clear revisit target',
)

chat_path = 'src/features/chat/ChatScreen.tsx'
replace_once(
    chat_path,
...
replace_once(
    ...
    'chat exact follow-up id',
)

note_view_path = 'src/features/notes/NoteViewSheet.tsx'
replace_once(
    note_view_path,
    "import {\n  formatFollowUpTimestamp,\n} from '../../domain/followUps';",
    "import {\n  formatFollowUpTimestamp,\n  getFollowUpPrompt,\n} from '../../domain/followUps';",
    'history prompt import',
)
replace_once(
    note_view_path,
    "<p>{record.prompt}</p>",
    "<p>{record.prompt || getFollowUpPrompt(record, note, language)}</p>",
    'history prompt fallback',
)

Path('tests/components/followUpScheduler.test.tsx').write_text("""import { act, renderHook } from '@testing-library/react';
...
Path('tests/unit/noteEditorFollowUps.test.ts').write_text("""import { describe, expect, it } from 'vitest';
...
Path('tests/components/followUpTargeting.test.tsx').write_text("""import { fireEvent, render, screen } from '@testing-library/react';
